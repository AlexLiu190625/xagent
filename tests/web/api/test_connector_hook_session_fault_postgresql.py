...
``poison_by_raw_statement`` (see test_mcp_reported_edit_permission.py) only
actually poisons PostgreSQL: a failed raw statement aborts the surrounding
transaction there, so every later statement on the same connection is
refused until a rollback runs -- SQLite does not enforce that the same way,
so the SQLite-backed suite cannot prove this shape needs the fix.
...
``_restore_session_after_hook_failure`` turns this test red on this file
specifically (confirmed by running it against a real server with that line
