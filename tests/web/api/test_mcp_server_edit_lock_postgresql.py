...
def test_a_row_that_vanishes_after_the_gate_but_before_the_lock_is_a_404_not_a_500(
    ...
    real_resolve = mcp_api._resolve_mcp_server_for_request

    def resolve_then_delete_concurrently(db_, user_id, sid):
        result = real_resolve(db_, user_id, sid)
        ...
        with session_factory() as other:
            other.execute(
                sa.delete(UserMCPServer).where(UserMCPServer.mcpserver_id == sid)
            )
            other.execute(sa.delete(MCPServer).where(MCPServer.id == sid))
            other.commit()
        return result

    mcp_api._resolve_mcp_server_for_request = resolve_then_delete_concurrently
    db = session_factory()
    try:
        ...
        mcp_api._resolve_mcp_server_for_request = real_resolve
        db.close()
