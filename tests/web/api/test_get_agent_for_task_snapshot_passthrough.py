...
Background:
    Step 3 introduced
    ``await asyncio.to_thread(load_task_setup_snapshot_sync, ...)``
    inside ``get_agent_for_task`` so the Task/Agent/LLM DB block ran
    off the main event loop. Step 4 plumbs the snapshot one layer
    upstream -- ``_schedule_bg._runner`` loads it once and hands it
    down through ``execute_task_background``. Without coordination
    the two would spawn redundant worker threads and re-read the
    same rows.
...
What this test pins:
    ...
    * When the caller supplies ``task_setup_snapshot``,
      ...
      the load-bearing test for "Step 4 wins are real, not undone".
    * When the caller passes ``task_setup_snapshot=None`` (WS / chat
      single-task / tests that don't know about the upstream
      snapshot), the Step-3 in-method thread call still runs
      exactly once.
"""
...
async def test_caller_supplied_snapshot_skips_internal_to_thread() -> None:
    """The Step 4 contract: if the caller already loaded a snapshot,
    ``get_agent_for_task`` must NOT call
    ``load_task_setup_snapshot_sync`` again. This protects the win --
    a regression that re-spins the worker thread would silently
    double the snapshot load cost per turn.
    """
...
async def test_no_snapshot_falls_back_to_internal_to_thread() -> None:
    ...
    assert loader_mock.call_count == 1, (
        ...
        f"fallback path, got {loader_mock.call_count}. If this regresses, "
        "either Step 3's fallback was removed (WS path broken) or the "
        "snapshot is being loaded twice."
    )
