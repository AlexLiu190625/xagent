...
class AgentToolResult(BaseModel):
    ...
    The classification fields below are only ever populated by a hand-built
    dict (never by instantiating this model) when the delegated child paused,
    failed, or the runtime hit an unexpected error; they are declared here so
    ``return_type()`` still describes the tool's full possible output shape.
    A normal completion keeps the legacy two-field shape untouched.
    """
