...
class AgentToolResult(BaseModel):
    """Result from a successful agent tool execution.

    Classified failures (a paused, failed, or errored delegated child) are
    returned as hand-built dicts, never instances of this model — see
    ``_classified_failure`` and its call sites. This model only describes
    the happy-path shape ``run_json_async`` actually constructs it with.
    """
