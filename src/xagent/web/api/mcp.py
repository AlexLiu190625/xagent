...
def update_mcp_server(
    ...
    try:
        ...
        if writes_definition_row:
            ...
            # Without a post-lock re-ask, a caller whose personal row vanished
            # during the wait has only the gate's verdict to fall back on.
            # None means the gate never found one either, which is the gate's
            # own 404 condition -- answer it the same way it would have.
            if is_stand_in and team_access is None:
                db.rollback()
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
