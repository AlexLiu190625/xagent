...
does increment a counter: the new
``COUNTER_ANCHOR_ABSENT_MISSING_RUN_PARTITION``. Steps 2 and 6 (no
checkpoint pointer, and a resolved anchor) are the two outcomes that
remain uninstrumented: nothing downstream depends on either rate today.
